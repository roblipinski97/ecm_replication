...
### based on the arguments specified below


cross_regression = function(var2,  # what outcome variable should be used
  ...
  if(group1 == 'Pure control'){
    m1 = lm(var_dummy ~   factor(block_categorical)  + age + gender + var_pre_dummy + ecm_include_patient, data = dta_reg) # Cross-sectional
    ...
    ### Store p-values
    # p1 = coeftest(m1, cluster.vcov(m1, dta_reg$list_id, df_correction = T))['ecm_include_patientControl',4]
    # p2 = coeftest(m2, cluster.vcov(m2, dta_reg$list_id, df_correction = T))['ecm_include_patientControl',4]
    
  }else if(group1 == 'Control'){
    m1 = lm(var_dummy ~  factor(strata)  + age + gender + var_pre_dummy + ecm_include_patient, weights = weight, data = dta_reg) # Cross-sectional
    m2 = lm(var ~  factor(strata)  + age + gender + var_pre + ecm_include_patient, weights = weight, data = dta_reg) # ANOVA (controlling for pre-treatment outcomes)
  
    ### Store p-values
    # p1 = coeftest(m1, cluster.vcov(m1, dta_reg$list_id, df_correction = T))['ecm_include_patientTreatment',4]
    # p2 = coeftest(m2, cluster.vcov(m2, dta_reg$list_id, df_correction = T))['ecm_include_patientTreatment',4]
  ...
  ### Estimate simples WLS for those variables for which ANCOVA is unreasonable (due to lack of pre-treatment values)
  if(var2 %in% c('consult_include', 'consult_care_plan', 'consult_refuse')){
    if(group1 == 'Pure control'){
      m1 = lm(var_dummy ~   factor(block_categorical)  + age + gender  + ecm_include_patient, data = dta_reg) # Cross-sectional
      m2 = lm(var  ~   factor(block_categorical)  + age + gender + ecm_include_patient, data = dta_reg) # ANOVA (controlling for pre-treatment outcomes)

    }else if(group1 == 'Control'){
      m1 = lm(var_dummy ~  factor(strata)  + age + gender  + ecm_include_patient, weights = weight, data = dta_reg) # Cross-sectional
      m2 = lm(var ~  factor(strata)  + age + gender + ecm_include_patient, weights = weight, data = dta_reg) # ANOVA (controlling for pre-treatment outcomes)
    }
  }
  ...
  m_all = left_join(m1 %>% rename('beta_dummy' = 'beta_se'),
                m2 %>% rename('beta_count' = 'beta_se')) %>% 
          mutate(var = var1) %>% 
...
### to create main regression (ANCOVA) table (Table 2 in the paper)
### An internal loop will also create 3 different versions of this table - one for 
### all patients in the ECM (Table 2 in the main body of the manuscript) and Tables
### A5 and A6 for the Appendix, separating the results for mild- and severe-risk patients
### only
...
count = 0
vars =  c('consult_care_plan', 'consult_gp', 'consult_nurse', 'consult_any_your')
vars = c('n_inpatient_any', 'n_outpatient_your', 'n_primary_your', 'n_diag', 'readmit_30_any')
vars =  c(  'heart_failure_any', 'stroke_any', 'myocardial_infarction_any', 'diabetes_2_any', 'hyperlipidemia', 'weight_high')
vars =  c('p_a_diabetes', 'p_c_beta_blockers', 'p_statins', 'p_c_hypertensive', 'p_key', 'p_other')
 vars =  c('m_glicihem_riigi', 'm_creatinine_riigi', 'm_chol_tri_riigi', 'm_glucose_riigi', 'm_thc_riigi')


### Loop across outcome variables  ---------------------------------------------------------------------------------------------------
for(year1 in c(2018)){
  
  ### Loop across risk sub-groups
  for(risk1 in c('all')){
  ...
  for(i in seq_along(vars) ){
    ...
    dfs1 = list(
      ...
      cross_regression(var1, 'Control', 'Treatment', year1, risk1) %>% 
        rename_with(.cols = -c(var), .fn = ~paste0(., '_34')),
      
      cross_regression(var1, 'Pure control', 'Control', year1, risk1) %>%
        rename_with(.cols = -c(var), .fn = ~paste0(., '_56'))
    )
    ...
    temp = temp[, c(1,
                    2, 3,
                    4, 5,
                    8, 9)] # Specify as c(1, 6, 7, 2, 3, 8, 4, 9, 5 ) if we want to keep dummy means too
  ...
  dta_table_save = rbindlist(temp_list) %>% dplyr::select(-c(starts_with('p_')))
 
  ### Assign FE + means + N    ---------------------------------------------------------------------------------------------------
  if(risk1 != 'all'){temp = dta_outcomes %>% filter(class_code == risk1)}else{temp = dta_outcomes}
  
  dta_table_save = dta_table_save %>% as.data.frame()
  dta_table_save[nrow(dta_table_save)+1, ] = c('\\textbf{FE}', '-', '-', 'Strata', 'Strata', 'Block', 'Block')
  dta_table_save[nrow(dta_table_save)+1, ] = c('\\textbf{Controls}',  '-', '-',
                                               '\\scriptsize{Age, gender, $DV_{18-21}$}','\\scriptsize{Age, gender, $DV_{18-21}$}',
                                               '\\scriptsize{Age, gender, $DV_{18-21}$}',  '\\scriptsize{Age, gender, $DV_{18-21}$}')
  dta_table_save[nrow(dta_table_save)+1, ] = c('\\textbf{N}', 
                                               prettyNum(n_distinct(temp$id[temp$ecm_include_patient %in% c('Control')]), big.mark = ','), 
                                               prettyNum(n_distinct(temp$id[temp$ecm_include_patient %in% c('Control')]), big.mark = ','), 
                                               prettyNum(n_distinct(temp$id[temp$ecm_include_patient %in% c('Control', 'Treatment')]), big.mark = ','), 
                                               prettyNum(n_distinct(temp$id[temp$ecm_include_patient %in% c('Control', 'Treatment')]), big.mark = ','), 
                                               prettyNum(n_distinct(temp$id[temp$ecm_include_patient %in% c('Pure control', 'Control')]), big.mark = ','), 
                                               prettyNum(n_distinct(temp$id[temp$ecm_include_patient %in% c('Pure control', 'Control')]), big.mark = ','))
  
  
  #if(year1 == 2018){fwrite(dta_table_save, file.path(project_path,  'Tables','CSV', risk1, 'table2_cross.csv'))}
  #if(year1 == 2009){fwrite(dta_table_save, file.path(project_path,  'Tables','CSV', risk1, 'table2_cross_09.csv'))}
  }
}
...
# if(year1 == 2018){dta_table_save = fread(file.path(project_path,  'Tables','CSV', risk1, 'table2_cross.csv'))}
# if(year1 == 2009){dta_table_save = fread(file.path(project_path, 'Tables','CSV', risk1, 'table2_cross_09.csv'))}
